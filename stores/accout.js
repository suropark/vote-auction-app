...
import { mainnetURI, providerOptions } from "./constants/options";
  ...
  connect: async () => {
    const providerOptions = {
      ...
      ["metamask"]: {
        ...
        package: null,
        connector: async (provider, options) => {
          // window.location.href = "https://metamask.app.link/dapp/meshed.finance/";
        },
      },
    };
    ...
    let newSendProvider;
    if (provider.isCaver) {
      // using window.klaytn ex) kaikas,
    } else if (provider.isKlip) {
    } else {
      // web3 using window.ethereum
    }

    set({
      sendProvider: newSendProvider,
      address: "",
      connected: true,
    });
  },
